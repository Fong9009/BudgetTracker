...
const upload = multer({
  ...
  fileFilter: (req: any, file: any, cb: any) => {
    ...
    if (!allowedMimeTypes.includes(file.mimetype)) {
      return cb(new Error('Only CSV, Excel, and PDF files are allowed'), false);
    }
    
    // Check file extension
    const fileExtension = file.originalname.toLowerCase().substring(file.originalname.lastIndexOf('.'));
    if (!allowedExtensions.includes(fileExtension)) {
      return cb(new Error('Invalid file extension. Please use .csv, .xlsx, or .pdf files'), false);
    }
    ...
    async (req: any, res) => {
      try {
        ...
        if (file.mimetype === 'text/csv' || file.mimetype === 'application/csv' || file.originalname.toLowerCase().endsWith('.csv')) {
          ...
          // Parse PDF file
          result = await PDFParser.parsePDFStatement(file.buffer);
        } else {
          return res.status(400).json({ error: 'Unsupported file type. Please upload CSV, Excel, or PDF files.' });
        }
